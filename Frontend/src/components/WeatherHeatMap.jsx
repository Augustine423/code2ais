...
export default function LiquidHeatmapFromAPI() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const res = await fetch('http://13.125.190.239:4000/api/datalogs');
  ...
  return (
    <div className='w-screen h-full flex justify-center items-center relative'>
      ...
      {sensorData  && sensorData.map((data,) => (
        <div className='w-3/12 absolute top-5 right-2 z-50 bg-opacity-40 bg-white backdrop-blur-md rounded-md py-5 px-4 shadow-xl flex flex-wrap items-center justify-center'>
